...
from manual_line_sim import StationSpec, BufferSpec, ManualLineSim, ExperimentHarness, default_line
...
with st.sidebar:
    ...
    sim_time_s = st.slider("sim_time_s", min_value=30, max_value=4_3200, value=3600, step=30)
...
buf5 = st.number_input("Buffer 5", min_value=0, max_value=50, value=1, step=1)  # NEW
    ...
    for i, s in enumerate(base_specs):
        ...
        mean_ct = st.number_input(f"mean_ct_{s.name}", min_value=5.0, max_value=120.0, value=float(s.mean_ct), step=1.0)
        cv = st.slider(f"cv_{s.name}", min_value=0.0, max_value=1.0, value=float(s.cv), step=0.05)
        dr = st.number_input(f"defect_risk_{s.name}", min_value=0.0, max_value=0.05, value=float(s.defect_risk), step=0.001, format="%.3f")
        station_specs.append(StationSpec(s.name, mean_ct=mean_ct, cv=cv, defect_risk=dr))

    run_col1, run_col2 = st.columns(2)
    with run_col1:
        start_btn = st.button("▶️ Start Simulation", width="stretch", type="primary")
    with run_col2:
...
tabs = st.tabs(["Production", "Buffers & Inventory", "Quality & Sensors", "Timeline", "Config Snapshot", "Scenarios (batch)"])
    ...
    if logs.empty:
        ...
        for si, name in enumerate([s.name for s in station_specs]):
            ...
            if len(merged):
                ...
                res_ct.append({"station": name, "mean_ct": ct["mean"], "p50": ct["50%"], "std": ct["std"], "n": len(merged)})
            else:
        ...
        with c2:
            st.bar_chart(df_ct.set_index("station")["mean_ct"], width="stretch")

        st.markdown("**Recent station events**")
        last_events = logs[logs["event"].isin(["start", "finish", "complete", "to_buffer"])].sort_values("t", ascending=False).head(30)
        st.dataframe(last_events[["t","event","station_name","item_id","cam_has_frame","cam_conf","HR","HRV","posture_risk"]], width="stretch", height=400)
...
for i in range(n_buffers):
            ...
            with buf_plots[i]:
                ...
                if len(dfb):
                    st.line_chart(dfb.set_index("t"), width="stretch")
                else:
...
with tabs[2]:
    ...
    if logs.empty:
        ...
        if len(qc):
            ...
            st.dataframe(qc[["t","item_id","defect","p_def"]].sort_values("t", ascending=False), width="stretch", height=260)
        else:
        ...
        if len(notif_df):
            st.dataframe(notif_df, width="stretch", height=260)
        else:
...
with tabs[3]:
    ...
    if logs.empty:
        ...
        st.line_chart(starts, width="stretch")
...
with tabs[5]:
   ...
   buf_str = st.text_input(
    ...
    sim_override = st.number_input("Simulation horizon for batch (sec)", min_value=30, max_value=43200, value=int(sim_time_s), step=30)
       ...
       parts = [int(x.strip()) for x in line.split(',')]; assert len(parts) == 5; buf_lists.append(parts)
        ...
        if valid:
            try:
                ...
                st.error("Check dropout/S3 lists; they must be numbers."); valid = False
        if valid:
            with st.status("Running batch scenarios…", expanded=True) as status:
                ...
                summary = harness.aggregate(results); wall = time.time() - t0
            ...
            st.markdown("**Top 10 by mean throughput**"); st.dataframe(top, width="stretch")
